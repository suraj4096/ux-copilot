...
- Tools: search_surveys, search_forms (optional surveyId), search_responses (requires formId), validate_form_json, open_form_editor. All search tools support offset, limit, and optional search text; use pagination when totals exceed the limit.
...
- Root object: "id" (non-empty string, use a new id like a UUID), "title" (non-empty string), optional "description" (string), "questions" (array with at least one question).
- At least one question must have "required": true. Every question needs a unique "id" (non-empty string).
- Question fields on every type: "id", "type", "label" (non-empty string), "required" (boolean).
...
- For "single_choice" and "multi_choice", include "options": an array of objects { "value": non-empty string, "label": non-empty string } (never a bare string list). Each option "value" must be unique within that question. At least one option per choice question.
